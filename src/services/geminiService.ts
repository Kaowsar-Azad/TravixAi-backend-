...
class GeminiService {
  ...
  private instances: GoogleGenerativeAI[];
   ...
   */
  async generateContentWithFallback(modelName: string, prompt: string, systemInstruction?: string, attempts: number = 0): Promise<string> {
   ...
   */
  async customizePlan(basePlanStr: string, userPreferences: string): Promise<string> {
    const prompt = `
...
`;
  ...
  async chatWithAgent(
    ...
    try {
      ...
      let messageParts: any = userMessage;
      if (file) {
        messageParts = [
          { text: userMessage },
